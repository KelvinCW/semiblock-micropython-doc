...
SemiBlock saves the **blocks themselves** — their types, positions, and field
values — not just the generated code. That means when you load a project, the
visual layout returns exactly as you left it, and the code regenerates from it.

Under the hood, SemiBlock uses Blockly's standard serialization to turn your
workspace into a JSON snapshot.
...
> Loading disables block events while it rebuilds, then re-enables them — so the
...
- Saving uses **browser local storage**, so a different browser or computer won't
  see the same saved project.
- Clearing your browser data also clears saved SemiBlock projects.
- For long-term backups, copy the generated MicroPython into a `.py` file you keep
  yourself.
